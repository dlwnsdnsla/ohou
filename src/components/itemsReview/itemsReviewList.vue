    ...
    <v-container fluid style="margin: 0;padding: 0;">
      ...
      :sort-desc="sortDesc" hide-default-footer>
        <template v-slot:header>
          <v-toolbar class="d-flex justify-center" color="#fff" flat>
            <v-toolbar-title class="headline" style="flex: 1;">리뷰✨</v-toolbar-title>
          </v-toolbar>
        </template>
  ...
  <script>
    ...
    export default {
      ...
      moveToWrite(){
        ...
        mounted() {
            this.listCount = this.oReviews.reverse().length
            this.init()
        },
        computed:{
          numberOfPages () {
