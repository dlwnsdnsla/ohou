<template>
    ...
    <v-card class="mx-auto">
      <v-toolbar color="#81D4FA" dark>
        <v-toolbar-title>구매 완료된 상품 {{ list.length }}개</v-toolbar-title>
      </v-toolbar>
        ...
        <template v-for="(item, index) in completelist">
          ...
          <v-list-item :key="item.title">
            ...
            <v-list-item-content class="d-flex">
              <v-list-item-title class="mb-5" style="font-size:18px;font-weight: bold;">{{ item.name }} </v-list-item-title>
              <v-list-item-title style="color:#868686">구매완료된 날짜 : {{ item.date }} </v-list-item-title>
            </v-list-item-content>
          </v-list-item>
...
<script>
    ...
    export default {
        ...
        created(){
           ...
           this.list = this.completelist.filter((item)=>item.userEmail == email)
